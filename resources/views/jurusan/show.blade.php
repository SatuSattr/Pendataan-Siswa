<x-app-layout>
    ...
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-brand-50 via-white to-slate-50 p-6 ring-1 ring-slate-200">
            ...
            <div class="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('jurusan.index') }}" class="inline-flex items-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-brand-300 hover:text-brand-700">
        ...
        <div class="card-surface rounded-2xl p-6 ring-1 ring-slate-100">
            ...
            @php
                $kelasList = $jurusan->kelas()
                    ->with('tahunAjar')
                    ->withCount(['kelasDetails as siswa_aktif_count' => function ($q) {
            ...
            <div class="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-6 text-center text-sm text-slate-500">
                ...
                <table class="min-w-full table-soft divide-y divide-slate-100">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="transition hover:bg-slate-50/60">
                            ...
                            <td class="px-4 py-3 text-slate-800">{{ $kelasItem->tahunAjar->kode_tahun_ajar ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-slate-800">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('kelas.show', $kelasItem) }}" class="text-brand-600 hover:text-brand-700" title="Detail">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                    <a href="{{ route('kelas.edit', $kelasItem) }}" class="text-sky-600 hover:text-sky-700" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('kelas.destroy', $kelasItem) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
