<div x-cloak x-show="showEditModal" class="fixed inset-0 z-[96] flex items-center justify-center px-4" role="dialog"
    ...
    <form x-show="showEditModal" x-transition method="POST" :action="`${updateActionBase}/${editForm.id}`"
        ...
        <div class="space-y-6">
            <div class="space-y-1">
                <p class="text-xl font-bold text-slate-900">Edit Jurusan</p>
                <p class="text-sm text-slate-600">Perbarui kode dan nama jurusan tanpa meninggalkan halaman.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="block text-sm font-semibold text-slate-700" for="edit_kode_jurusan">Kode</label>
                    <input type="text" name="kode_jurusan" id="edit_kode_jurusan" x-ref="editKodeInput"
                        x-model="editForm.kode_jurusan" required
                        class="mt-2 block w-full rounded-xl border-slate-200 text-slate-800 shadow-sm focus:border-brand-400 focus:ring-brand-200">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700" for="edit_nama_jurusan">Nama</label>
                    <input type="text" name="nama_jurusan" id="edit_nama_jurusan" x-model="editForm.nama_jurusan"
                        required
                        class="mt-2 block w-full rounded-xl border-slate-200 text-slate-800 shadow-sm focus:border-brand-400 focus:ring-brand-200">
                </div>
            </div>
