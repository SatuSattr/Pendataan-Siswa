<div x-cloak x-show="showAddModal" class="fixed inset-0 z-[95] flex items-center justify-center px-4" role="dialog"
    ...
    <form x-show="showAddModal" x-transition x-ref="addFormEl" method="POST" action="{{ route('jurusan.store') }}"
        ...
        <div class="space-y-6">
            <div class="space-y-1">
                <p class="text-xl font-bold text-slate-900">Tambah Jurusan</p>
                <p class="text-sm text-slate-600">Buat jurusan baru langsung dari halaman daftar.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="block text-sm font-semibold text-slate-700" for="add_kode_jurusan">Kode</label>
                    <input type="text" name="kode_jurusan" id="add_kode_jurusan" x-ref="addKodeInput"
                        value="{{ old('kode_jurusan') }}" placeholder="Contoh: TKJ" required
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700" for="add_nama_jurusan">Nama</label>
                    <input type="text" name="nama_jurusan" id="add_nama_jurusan" value="{{ old('nama_jurusan') }}"
