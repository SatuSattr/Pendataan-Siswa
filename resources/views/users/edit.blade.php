<x-app-layout>
    <x-slot name="header">
        <x-page-header path="/users/edit/{{ $user->name }}" title="Edit Pengguna">
            <x-slot name="actions">
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_440px] xl:grid-cols-[minmax(0,1fr)_520px]">
        <div class="hidden lg:block">
            <div class="glass-panel rounded-3xl p-6 shadow-lg">
                <h3 class="text-lg font-semibold text-slate-900">Kelola Akun</h3>
                <p class="mt-2 text-sm text-slate-600">Perbarui data pengguna dan perannya tanpa mengganggu akses yang sudah berjalan.</p>
                <ul class="mt-4 space-y-2 text-sm text-slate-700">
                    <li class="flex items-start gap-2">
                        <span class="mt-1 h-2.5 w-2.5 rounded-full bg-brand-400"></span>
                        <span>Jaga konsistensi email untuk notifikasi dan reset password.</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="mt-1 h-2.5 w-2.5 rounded-full bg-brand-200"></span>
                        <span>Password baru bersifat opsional; kosongkan jika tidak diganti.</span>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="glass-panel sticky top-6 rounded-3xl p-6 shadow-xl lg:max-h-[calc(100vh-140px)] lg:overflow-y-auto">
                ...
                <form action="{{ route('users.update', $user) }}" method="POST" class="mt-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" class="mt-2 block w-full rounded-xl border-slate-200 shadow-sm focus:border-brand-400 focus:ring-brand-400" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" class="mt-2 block w-full rounded-xl border-slate-200 shadow-sm focus:border-brand-400 focus:ring-brand-400" required>
                        @error('email')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-800">Password Baru (opsional)</label>
                            <input type="password" name="password" class="mt-2 block w-full rounded-xl border-slate-200 shadow-sm focus:border-brand-400 focus:ring-brand-400">
                            @error('password')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password_confirmation" class="mt-2 block w-full rounded-xl border-slate-200 shadow-sm focus:border-brand-400 focus:ring-brand-400">
                    </div>
